...
public class ChairController {
	...
	public ModelAndView newClubForm(@RequestParam("regNo") String regNo,
		...
		if (it.hasNext()) {
			...
			mav1.addObject("Mess", "Ooops!!Registration Number Does Not Exist.");
			return mav1;
		}
	...
	public ModelAndView create(@Valid @ModelAttribute("newChair") ChairModel chair,
		...
		while(it1.hasNext()) {
			...
			mav.addObject("successMess", "Oops!!...A student With That Regno Already Exists");
			return mav;
		}
		...
		String fname="",lname="",sname="",position="",clubname="",regnum="";
		String cellphone="";
		...
		if (i > 0) {
			...
			mm.setLname(lname);
			mm.setSname(sname);
			mm.setPosition("Chairperson");
			mm.setEmailaddress(emailadress);
			mm.setCellphone(cellphone);
			mm.setClubName(clubname);
			mm.setYearOfRegistration(year);
	...
	public String update(@ModelAttribute("editChair") ChairModel chair,
		...
		int updateEntities = sess.createQuery(hqlUpdate).setString("fn", chair.getFirst_name())
				...
				.setString("cp",chair.getClubName()).setString("cn", chair.getClubName()).executeUpdate();
		tx.commit();
		sess.close();
