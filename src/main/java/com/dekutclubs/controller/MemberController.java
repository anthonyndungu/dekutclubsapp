...
public class MemberController {
	...
	public ModelAndView searchClubs(@RequestParam(required = false, defaultValue = "") String regno) {
	...
	public String viewClubOfficials(@Valid SelectClubModel scm, BindingResult result, SessionStatus status,
		...
		Query qry = sess
				.createQuery("from MemberModel m where m.clubName=? and m.yearOfRegistration=? " + "and m.position=?");
	...
	public String DisApprove(@Valid SelectionModel sm, BindingResult result, SessionStatus status,
			HttpServletRequest request) throws Exception {
		...
		Query qry = sess.createQuery("from MemberModel m where m.clubName=:cn and m.yearOfRegistration=:yor");
	...
	public ModelAndView create(@Valid @ModelAttribute("newMember") MemberModel member, BindingResult result,
		...
		Session sess = factory.openSession();
		String clubName = "";
		Query qry = sess.createQuery("from ChairModel p where p.chairid=:id");
		qry.setInteger("id", idChair);
		List l = qry.list();
		Iterator it = l.iterator();
		...
		SQLQuery query1 = sess.createSQLQuery("select u.FirstName from members u where u.CellPhone=:pno");
		query1.setString("pno", member.getCellphone());
		List list1 = query1.list();
		Iterator it2 = list1.iterator();
		while (it2.hasNext()) {
			ModelAndView mav = new ModelAndView("newMember");
			mav.addObject("successMess", "Member with phone number. " + member.getCellphone() + " already exist!");
			return mav;
		}
		SQLQuery query2 = sess.createSQLQuery("select u.FirstName from members u where u.RegNo=:reg ");
		query2.setString("reg", member.getRegno());
		List list2 = query2.list();
		Iterator it1 = list2.iterator();
		while (it1.hasNext()) {
			ModelAndView mav = new ModelAndView("newMember");
			mav.addObject("successMess", "Member with reg no. " + member.getRegno() + " already exists!");
			return mav;
		}
	...
	public ModelAndView update(@ModelAttribute("editMember") MemberModel member, BindingResult result,
			SessionStatus status,HttpSession session) {
		validator.validate(member, result);
		if (result.hasErrors()) {
			ModelAndView mav = new ModelAndView("editMember");
