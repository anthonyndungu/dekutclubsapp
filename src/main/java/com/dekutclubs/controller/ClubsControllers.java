...
public class ClubsControllers {
	...
	public ModelAndView searchClubs(@RequestParam(required = false, defaultValue = "") String clubname) {
	...
	public String getDeactivatedClub(HttpServletRequest request,HttpSession session) {
	...
	public ModelAndView newclubForm(HttpServletRequest request,HttpSession session) {
		...
		request.setAttribute("clubs", clubName);
		request.setAttribute("chair", chairName);
		request.setAttribute("contact", contact);
		request.setAttribute("email", emailAddress);
		
		SessionFactory fact = new AnnotationConfiguration().configure("hibernate.cfg.xml").buildSessionFactory();
		Session ses = fact.openSession();

		Query qry1 = ses.createQuery("select c.categoryname from ClubCategoryModel c");
		List l1 = qry1.list();
	...
	public ModelAndView create(@ModelAttribute("newClub") ClubModel club,BindingResult result, SessionStatus status,
		...
		if (result.hasErrors()) {
			...
			Query qry1 = sess.createQuery("select c.categoryname from ClubCategoryModel c");
			List l1 = qry1.list();
			request.setAttribute("category", l1);
		...
		while (it.hasNext()) {
			Object o = (Object) it.next();
			ChairModel p = (ChairModel) o;
			clubnam=p.getClubName();
		}
		String cln="";
		Query qry1 = sess.createQuery("from ClubModel c where c.clubname=:cid");
		qry1.setString("cid", clubnam);
		List l1 = qry1.list();
		...
		if(cln!=""){
			...
			Query qury = sess.createQuery("from ChairModel c where c.chairid=:cid");
			...
			Query qry2 = sess.createQuery("select c.categoryname from ClubCategoryModel c");
			List l2 = qry2.list();
		...
		Query query = session.createQuery("from ClubModel c where c.clubid=?");
		...
		mav.addObject("successMess", "Data Saved Successfully");		
	...
	public String activate(@RequestParam("clubid") Integer clubid) {
		...
		int chairid = 0;
		Query query1 = session.createQuery("from ChairModel a where a.clubName=?");
		...
		int updateEntities = session.createQuery(hqlUpdate).setString("status", "Approved").setInteger("lgid", chairid)
				.executeUpdate();
	
		String hqlUpdate1 = "update ClubModel c set c.status=:status where c.clubname=:cname";
		int updateEntities1 = session.createQuery(hqlUpdate1).setString("status", "Active").setString("cname", clubName)
				.executeUpdate();
		String hqlUpdate2 = "update UploadFile u set u.status=:status where u.clubid=:cname";
		int updateEntities2 = session.createQuery(hqlUpdate2).setString("status", "Active").setString("cname", clubName)
				.executeUpdate();
	...
	public ModelAndView update(@ModelAttribute("editClub") ClubModel contact, BindingResult result, SessionStatus status) {
